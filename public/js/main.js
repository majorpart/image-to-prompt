...
class ImageToPromptApp {
    ...
    init() {
        ...
        // 延迟调用 generateTestimonials，确保 DOM 已完全渲染（特别是通过 dangerouslySetInnerHTML 注入的内容）
        setTimeout(() => {
    ...
    generateTestimonials() {
        ...
        // 使用重试机制，确保 DOM 元素已准备好
        const tryGenerateTestimonials = (attempt = 0, maxAttempts = 5) => {
            ...
            console.log(`[Testimonials] Attempt ${attempt + 1}: Looking for columns:`, {
                column1: !!column1,
                column2: !!column2,
                column3: !!column3
            });
            ...
            if (column1 && column2 && column3) {
                ...
                console.log('[Testimonials] Successfully generated testimonials:', {
                    column1: column1Testimonials.length,
                    column2: column2Testimonials.length,
                    column3: column3Testimonials.length
                });
            } else {
                if (attempt < maxAttempts - 1) {
                    // 如果还没找到，等待一段时间后重试
                    console.log(`[Testimonials] Columns not found, retrying in 200ms...`);
                    setTimeout(() => {
                        tryGenerateTestimonials(attempt + 1, maxAttempts);
                    }, 200);
                } else {
                    console.error('[Testimonials] Failed to find columns after', maxAttempts, 'attempts', {
                        column1: !!column1,
                        column2: !!column2,
                        column3: !!column3,
...
// 确保 ImageToPromptApp 类在全局可用
if (typeof window !== 'undefined') {
    window.ImageToPromptApp = ImageToPromptApp;
    console.log('[Main.js] ImageToPromptApp class exported to window');
}
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            if (!window.imageToPromptApp && window.ImageToPromptApp) {
                ...
                console.log('[Main.js] ImageToPromptApp initialized via DOMContentLoaded');
            }
        });
    } else {
        // DOM 已经加载完成，直接初始化
        if (!window.imageToPromptApp && window.ImageToPromptApp) {
            window.imageToPromptApp = new ImageToPromptApp();
            console.log('[Main.js] ImageToPromptApp initialized immediately (DOM ready)');
        }
    }
}
